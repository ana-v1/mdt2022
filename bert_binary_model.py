...
class Bert_Classifier(torch.nn.Module):
    def __init__(self, freeze_bert=False, input_shape=(768, 50, 2)):
        super(Bert_Classifier, self).__init__()

        self.bert = BertModel.from_pretrained('bert-base-uncased')
        self.cnt = 0
        H, hidden_layers, num_of_classes = input_shape

        # MLP
        self.linear1 = nn.Linear(H, hidden_layers)
        self.linear2 = nn.Linear(hidden_layers, num_of_classes)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        logits = self.drop(self.relu(self.linear2(first_linear_outputs)))
...
def train(model, train_dataloader, val_dataloader=None, epochs=3, batch_size=32, verbose=False, evaluation=False):
    ...
    for epoch_i in range(epochs):
        ...
        torch.save(bert_classifier, f'./bert_binary_model_epoch_{epoch_i}_acc_{val_accuracy}.pt')
...
if __name__ == "__main__":
    ...
    torch.save(bert_classifier, './bert_binary_model.pt')